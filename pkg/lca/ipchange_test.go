...
func TestNewIPConfigBuilder(t *testing.T) {
	testCases := []struct {
		name        string
		expectedErr string
		expectNil   bool
		...
		{
			name:        "ipconfig2",
			expectedErr: "IPConfig name must be " + ipConfigName,
			expectNil:   false,
			apiClient:   clients.GetTestClients(clients.TestClientParams{}),
		},
		{
	...
	for _, testCase := range testCases {
		testBuilder := NewIPConfigBuilder(testCase.apiClient, testCase.name)
...
func TestIPConfigCreate(t *testing.T) {
	testCases := []struct {
		ipconfig      *IPConfigBuilder
		expectedError error
	}{
		{
			ipconfig:      buildValidIPConfigBuilder(clients.GetTestClients(clients.TestClientParams{})),
			expectedError: nil,
		},
		{
			ipconfig:      buildInValidIPConfigBuilder(clients.GetTestClients(clients.TestClientParams{})),
			expectedError: fmt.Errorf("IPConfig name must be " + ipConfigName),
		},
	}

	for _, testCase := range testCases {
		testIPConfigBuilder, err := testCase.ipconfig.Create()
		assert.Equal(t, testCase.expectedError, err)

		if testCase.expectedError == nil {
			assert.Equal(t, testIPConfigBuilder.Definition.Name, testIPConfigBuilder.Object.Name)
		}
	}
}
...
func TestIPConfigPull(t *testing.T) {
	testCases := []struct {
		...
		{
			name:                "ipconfig2",
			expectedError:       fmt.Errorf("ipconfig object ipconfig2 does not exist"),
	...
	for _, testCase := range testCases {
		...
		builderResult, err := PullIPConfig(testSettings, testCase.name)
...
func TestIPConfigWithIPv4Address(t *testing.T) {
	testCases := []struct {
		...
		{
			ipv4Address: "invalid",
			expectedErr: "invalid IP argument invalid",
		},
	}
...
func TestIPConfigWithStage(t *testing.T) {
	testBuilder := buildValidIPConfigBuilder(clients.GetTestClients(clients.TestClientParams{}))
...
func TestIPConfigWithDNS(t *testing.T) {
	testCases := []struct {
		...
		{
			dnsServers:  []string{},
			expectedErr: "The DNS servers list cannot be empty",
		},
		{
			dnsServers:  []string{" "},
...
func TestIPConfigWaitUntilComplete(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		_, err := testIPConfigBuilder.WaitUntilComplete(time.Second * 1)
...
func buildTestIPConfigBuilderWithFakeObjects() *IPConfigBuilder {
	return NewIPConfigBuilder(
		buildIPConfigTestClientWithDummyObject(buildDummyIPConfigRuntime()), ipConfigName)
}
...
func buildValidIPConfigBuilder(apiClient *clients.Settings) *IPConfigBuilder {
	return NewIPConfigBuilder(apiClient, ipConfigName)
}

func buildValidIPConfigBuilderWithStacks(apiClient *clients.Settings) *IPConfigBuilder {
	builder := NewIPConfigBuilder(apiClient, ipConfigName)
	if builder == nil {
		return nil
	}
...
func buildInValidIPConfigBuilder(apiClient *clients.Settings) *IPConfigBuilder {
	return NewIPConfigBuilder(apiClient, "")
}
