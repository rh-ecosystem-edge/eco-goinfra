...
func NewRouteAdvertisementBuilder(
	...
	if err != nil {
		...
		glog.V(100).Infof("SUCCESS: Attached OVN scheme to client")
	}
...
func (builder *RouteAdvertisementBuilder) Create() (*RouteAdvertisementBuilder, error) {
	...
	if !builder.Exists() {
		...
		glog.V(100).Infof("About to call apiClient.Create with Definition: %+v", builder.Definition)

		err = builder.apiClient.Create(context.TODO(), builder.Definition)

		if err != nil {
			glog.V(100).Infof("FAILED to create RouteAdvertisement %s: %v", builder.Definition.Name, err)
			glog.V(100).Infof("Error type: %T", err)
			return builder, fmt.Errorf("failed to create RouteAdvertisement %s: %w", builder.Definition.Name, err)
		} else {
			glog.V(100).Infof("SUCCESS: apiClient.Create returned no error for %s", builder.Definition.Name)
		}
