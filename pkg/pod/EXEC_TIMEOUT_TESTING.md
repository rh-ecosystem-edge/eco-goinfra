...
The tests in `pod_test.go` verify that the timeout parameter is properly enforced during:
1. Command execution phase
2. Connection establishment phase (future work)
3. Various timeout scenarios
4. Multi-container pods
5. Concurrent execution under load
...
2. **TestExecCommandWithTimeoutConnectionPhase**
   - Currently skipped (requires network simulation tools)
   - Placeholder for future connection-phase timeout testing

3. **TestExecCommandWithTimeoutMultipleContainers**
   - Tests timeout behavior with multi-container pods
   - Verifies timeout works correctly when targeting specific containers

4. **TestExecCommandWithTimeoutStressTest**
   - Runs 5 concurrent commands with timeouts
   - Verifies timeout enforcement under concurrent load

### Expected Test Duration

- **Full test suite:** ~3-5 minutes
- **Single test:** ~1-2 minutes
- Tests include pod creation, waiting for pod ready, command execution, and cleanup
...
Edit `pod_test.go` constants:

```go
const (
    containerImage  = "quay.io/centos/centos:stream9"  // Container image
    timeoutDuration = 90 * time.Second  // Global test timeout
)
```
...
go test -v -tags=integration ./integration/ -run TestExecCommandWithTimeoutStressTest -timeout 10m

# Run only multi-container tests
go test -v -tags=integration ./integration/ -run TestExecCommandWithTimeoutMultipleContainers -timeout 10m
