...
const (
	// Constants for pod integration tests
	podContainerImage  = "quay.io/centos/centos:stream9"
	podTimeoutDuration = 90 * time.Second
)
...
func TestPodCreate(t *testing.T) {
	...
	testContainerBuilder := pod.NewContainerBuilder("test", podContainerImage, []string{"sleep", "3600"})
	containerDefinition, err := testContainerBuilder.GetContainerCfg()
	assert.Nil(t, err)

	podBuilder := pod.NewBuilder(client, podName, testNamespace, podContainerImage)
	podBuilder = podBuilder.RedefineDefaultContainer(*containerDefinition)

	// Create a pod in the namespace
	_, err = podBuilder.CreateAndWaitUntilRunning(podTimeoutDuration)
	assert.Nil(t, err)
...
func TestExecCommandWithTimeoutEnforcement(t *testing.T) {
	...
	defer func() {
		err := nsBuilder.DeleteAndWait(podTimeoutDuration)
	...
	containerDef, err := CreateTestContainerDefinition("test-container",
		podContainerImage, []string{"sleep", "3600"})
	...
	podBuilder, err = podBuilder.CreateAndWaitUntilRunning(podTimeoutDuration)
	assert.Nil(t, err, "Failed to create and start pod")

	// Ensure pod is deleted at the end
	defer func() {
		_, err := podBuilder.DeleteAndWait(podTimeoutDuration)
